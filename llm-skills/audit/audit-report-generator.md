...
scored_gaps: [ { rule_id, severity, likelihood, risk, rationale }, ... ]
...
`fail` if any gap has risk at or above `min_report_severity`; otherwise `pass`.
...
scored_gaps:
  - { rule_id: RULE-destroy-after-purpose, severity: high, likelihood: high, risk: high, rationale: "No procedure." }
  - { rule_id: RULE-retention-notice, severity: high, likelihood: medium, risk: medium, rationale: "Ambiguous notice." }
...
audit_report:
  ...
  findings:
    ...
    - { rule_id: RULE-retention-notice, status: partial, risk: medium, gap: "보관기간 고지 불명확", remediation: "정보주체 고지 조항 명시", evidence: [C-001] }
