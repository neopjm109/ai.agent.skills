...
description: Parses a Markdown document into a structured document of requirements, architecture notes, decisions, constraints, and entities for downstream orchestration.
version: 1.0.0
...
requirements, architecture notes, design decisions, constraints, and entities —
and emit a normalized `structured_document`. This skill performs **analysis
...
- This skill parses `.md` only. Route `.docx` to `docs-analyze-docx`, `.pptx` to `docs-analyze-pptx`, and `.xlsx` to `docs-analyze-xlsx`.
