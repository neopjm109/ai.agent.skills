...
fixtures), a translation catalog, a knowledge base (FAQ/onboarding/glossary), or an analysis
report — into the correct operation and route it. This skill **never edits data directly**;
...
Every operation ends by re-running the domain's own validator (`seed-data-validator` /
`localization-validator`) so integrity is a deterministic pass/fail, not an assumption.
