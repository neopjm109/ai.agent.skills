...
design_system:
  design_tokens: { radius: { md: 12 }, spacing: { sm: 8 } }
```
...
- Flutter-only; React components are produced by `frontend/*` and are not reused here.

# Examples
Input:
```yaml
component_specs: [ { name: RatingStars, props: [value: double, max: int] } ]
design_system: { design_tokens: { colors: { star: "#FFB400" } } }
```
Output:
```yaml
flutter_widgets:
