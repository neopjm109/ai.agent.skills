...
- Flutter-only; React forms are produced by `frontend/*` and are not reused.

# Examples
Input:
```yaml
form_specs: [ { name: login_form, fields: [ { name: email, type: email, required: true } ], submit: authProvider.login } ]
```
Output:
```yaml
flutter_forms:
  file: lib/features/auth/login_form.dart
  code: |
