...
sequences, and reports. The target stack is fixed: Spring Boot + Next.js.
...
generated_artifacts: backend + frontend source
...
Invoke `spring-initializer` and `nextjs-initializer` **once** (the stack is fixed: Spring Boot +
Next.js) → build files, package structure, base config/profiles, and app skeleton. This scaffold
must exist before feature code is generated into it. Skip only when scaffolding a project the
run already targets.
...
For each feature, invoke `feature-orchestrator` (which fans out to backend/frontend/integration).
...
notes from the changeset. It returns self-healed (style-passing) `doc_artifacts`. Runs after
